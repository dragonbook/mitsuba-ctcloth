 ...
 * }
 */
class SimpleVolumetricPathTracerEx : public MonteCarloIntegrator {
public:
	SimpleVolumetricPathTracerEx(const Properties &props) : MonteCarloIntegrator(props)
    {
        m_isoDepth = props.getInteger("isoDepth", -1);
        m_singleNode = props.getBoolean("singleNode", true);

        if ( m_isoDepth > 0 )
            Log(EInfo, "isoDepth = %d", m_isoDepth);
    }

	/// Unserialize from a binary data stream
	SimpleVolumetricPathTracerEx(Stream *stream, InstanceManager *manager)
	 : MonteCarloIntegrator(stream, manager)
    {
        m_isoDepth = stream->readInt();
        m_singleNode = stream->readBool();
    }
		...
		while (rRec.depth <= m_maxDepth || m_maxDepth < 0) {
			...
			if (rRec.medium && rRec.medium->sampleDistance(Ray(ray, 0, its.t), mRec, rRec.sampler)) {
				...
				*/
                ...
                ++nscatter;
                if (  nscatter == m_isoDepth || 
                     (!m_singleNode && m_isoDepth >= 0 && nscatter > m_isoDepth) )
                {
                    pRec.wo = Warp::squareToUniformSphere(rRec.sampler->next2D());
                }
                else
                {
	...
	void serialize(Stream *stream, InstanceManager *manager) const {
        ...
        stream->writeInt(m_isoDepth);
        stream->writeBool(m_singleNode);
	}

	std::string toString() const {
		std::ostringstream oss;
		oss << "SimpleVolumetricPathTracer2[" << endl
			<< "  maxDepth = " << m_maxDepth << "," << endl
			<< "  rrDepth = " << m_rrDepth << "," << endl
			<< "  strictNormals = " << m_strictNormals << endl
            << "  isoDepth = " << m_isoDepth << endl
            << "  singleNode = " << m_singleNode << endl
			<< "]";
		return oss.str();
	}
...
MTS_IMPLEMENT_CLASS_S(SimpleVolumetricPathTracerEx, false, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(SimpleVolumetricPathTracerEx, "Simple volumetric path tracer 2");
MTS_NAMESPACE_END
