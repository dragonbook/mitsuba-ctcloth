...
class BinaryTextureMapVolume : public VolumeDataSourceEx {
public:
	enum EVolumeType {
		EFloat32 = 1,
		EFloat16 = 2,
	...
	BinaryTextureMapVolume(const Properties &props) : VolumeDataSourceEx(props), m_ready(false) {
		...
		if (props.hasProperty("fillRotateXY")) {
			float angle = props.getFloat("fillRotateXY");
		...
		m_textureFile = props.getString("textureFile");
		m_textureReso.x = props.getInteger("textureX");
		m_textureReso.y = props.getInteger("textureY");
		m_albedo2 = props.getSpectrum("albedo2");
	}


	BinaryTextureMapVolume(Stream *stream, InstanceManager *manager) 
	...
	virtual ~BinaryTextureMapVolume() {
	...
	void configure()
    {
        if ( !m_ready )
        {
			...
			//
			if (m_textureFile != "") {
				ref<FileStream> fs = new FileStream(m_textureFile, FileStream::EReadOnly);
//				int sz = fs->readInt();
//				if (sz != m_textureReso.x * m_textureReso.y) {
//					Log(EError, "Texture information size mismatch: expected %d but got %d", m_textureReso.x*m_textureReso.y, sz);
//				}
				int sz = m_textureReso.x * m_textureReso.y;
				m_texture = new bool[sz];
				fs->readArray(m_texture, sz); // check it? numpy dimensiton?
			}
			...
			m_worldToTexture =
				Transform::scale(Vector(
					static_cast<Float>(m_textureReso.x) / volumeExtents.x,
					static_cast<Float>(m_textureReso.y) / volumeExtents.y,
					1.0f / volumeExtents.z
					))*
				Transform::translate(Vector(
					-m_volumeAABB.min.x, -m_volumeAABB.min.y, -m_volumeAABB.min.z)) *
...
protected:
	...
	inline bool checkTexture(const Point &_p) const {
		Point p = m_worldToTexture.transformAffine(_p);
		int tx = floorToInt(p.x), ty = floorToInt(p.y);

		if (tx < 0 || tx >= m_textureReso.x || ty < 0 || ty >= m_textureReso.y || p.z < 0.0f || p.z > 1.0f)
			return false; // return value is meaningless

		return m_texture[ty*m_textureReso.x + tx];
	}
...
MTS_IMPLEMENT_CLASS_S(BinaryTextureMapVolume, false, VolumeDataSourceEx);
MTS_EXPORT_PLUGIN(BinaryTextureMapVolume, "Binary texture map volume");
MTS_NAMESPACE_END
