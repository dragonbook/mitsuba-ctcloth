...
class ImageMapVolumeLow : public VolumeDataSourceEx {
public:
	enum EVolumeType {
		EFloat32 = 1,
		EFloat16 = 2,
	...
	ImageMapVolumeLow(const Properties &props) : VolumeDataSourceEx(props), m_ready(false)
    {
	...
	ImageMapVolumeLow(Stream *stream, InstanceManager *manager) 
	...
	void configure()
    {
        if ( !m_ready )
        {
			...
			m_map = new uint8_t[m_mapReso.x*m_mapReso.y*2];
			fs->read(&m_map[0], m_mapReso.x*m_mapReso.y * 2);
...
MTS_IMPLEMENT_CLASS_S(ImageMapVolumeLow, false, VolumeDataSourceEx);
MTS_EXPORT_PLUGIN(ImageMapVolumeLow, "Image map volume low");
MTS_NAMESPACE_END
